...
if __name__ == '__main__':
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        # ===== overhead profiling (round-level) =====
        ...
        is_updated = True
        aggr_method = str(helper.params.get('aggregation_methods', config.AGGR_MEAN)).lower()
        if aggr_method == config.AGGR_MEAN:
            # Average the models
            ...
            num_oracle_calls = 1
        elif aggr_method == config.AGGR_GEO_MED:
        ...
        elif aggr_method == config.AGGR_FOOLSGOLD:
            is_updated, names, weights, alphas = helper.foolsgold_update(helper.target_model, updates)
            vis_agg_weight(helper,names,weights,epoch,vis,adversarial_name_keys)
            vis_fg_alpha(helper,names,alphas,epoch,vis,adversarial_name_keys )
            num_oracle_calls = 1
        elif aggr_method == config.AGGR_FLTRUST:


            is_updated, names, trust_scores = helper.fltrust_update(
                helper.target_model, updates, root_loader=None
            )

            vis_agg_weight(helper, names, trust_scores, epoch, vis, adversarial_name_keys)
            num_oracle_calls = 1
        elif aggr_method == config.AGGR_FLDETECTOR:
            is_updated, names, scores = helper.fldetector_update(helper.target_model, updates)
            vis_agg_weight(helper, names, scores, epoch, vis, adversarial_name_keys)
            num_oracle_calls = 1
        elif aggr_method == config.AGGR_LEADFL:
            is_updated, names, scores = helper.leadfl_update(helper.target_model, updates)
            vis_agg_weight(helper, names, scores, epoch, vis, adversarial_name_keys)
            num_oracle_calls = 1
        elif aggr_method == config.AGGR_FLIP:
            is_updated, names, scores = helper.flip_update(helper.target_model, updates)
            vis_agg_weight(helper, names, scores, epoch, vis, adversarial_name_keys)
            num_oracle_calls = 1
        else:
            logger.info(f"Unknown aggregation method '{aggr_method}', fallback to mean")
            is_updated = helper.average_shrink_models(weight_accumulator=weight_accumulator,
                                                      target_model=helper.target_model,
                                                      epoch_interval=helper.params['aggr_epoch_interval'])
            num_oracle_calls = 1

        round_wall_s = time.perf_counter() - round_wall_t0
        round_peak_gpu_mib = None
        if profile_on and torch.cuda.is_available():
            round_peak_gpu_mib = torch.cuda.max_memory_allocated() / (1024**2)

        if profile_on and (epoch - helper.start_epoch) < profile_rounds:
            logger.info(f"[OVERHEAD][ROUND] epoch={epoch} wall_s={round_wall_s:.4f} peak_gpu_mib={round_peak_gpu_mib}")
